...
func (plugin *CinderPlugin) CreateVolume(name string, size int64) (*api.VolumeSpec, error) {
...
func (plugin *CinderPlugin) GetVolume(volID string) (*api.VolumeSpec, error) {
	volumeService, err := plugin.getVolumeService()
	if err != nil {
		log.Println("Cannot access volume service:", err)
		return &api.VolumeSpec{}, err
...
func (plugin *CinderPlugin) CreateSnapshot(name, volID, description string) (*api.VolumeSnapshotSpec, error) {
	//Get the certified volume service.
	volumeService, err := plugin.getVolumeService()
	if err != nil {
		log.Println("Cannot access volume service:", err)
...
func (plugin *CinderPlugin) GetSnapshot(snapID string) (*api.VolumeSnapshotSpec, error) {
	volumeService, err := plugin.getVolumeService()
	if err != nil {
		log.Println("Cannot access volume service:", err)
		return &api.VolumeSnapshotSpec{}, err

...
func NewControllerWithVolumeConfig(
	...
	if vol != nil {
		prf, err := SearchProfile(vol.GetProfileId())
		if err != nil {
...
func (c *Controller) CreateVolume() (*api.VolumeSpec, error) {
	...
	polInfo, err := SearchSupportedPool(c.profile.Extra)
	if err != nil {
	...
	dckInfo, err := SearchDockByPool(polInfo)
	if err != nil {
		log.Printf("[Error] When search supported dock resource:", err)
		return &api.VolumeSpec{}, err
	}
...
func (c *Controller) DeleteVolume() *api.Response {
	...
	dckInfo, err := SearchDockByVolume(c.request.GetVolumeId())
	if err != nil {
		log.Printf("[Error] When search supported dock resource:", err)
		return &api.Response{
			Status: "Failure",
